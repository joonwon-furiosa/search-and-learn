...
def load_aime_combined() -> List[Dict[str, str]]:
    """
    Load AIME 2024 and 2025 train splits, keep minimal fields,
    and return a simple Python list of dicts without using concatenate_datasets.
    """
    ds1 = load_dataset("HuggingFaceH4/aime_2024")["train"]
    ds2 = load_dataset("yentinglin/aime_2025")["train"]

    def to_minimal(ds):
        # Drop auxiliary index column if present
        if "__index_level_0__" in ds.column_names:
            ds = ds.remove_columns("__index_level_0__")
        # Keep only fields we actually use
        # (problem for prompt, answer for accuracy check)
        return [{"problem": ex["problem"], "answer": ex["answer"]} for ex in ds]

    combined = to_minimal(ds1) + to_minimal(ds2)
    return combined
...
def main():
    ...
    dataset = load_aime_combined()

    n_values = [1, 8, 16, 32, 64] # [2, 4, 16, 32]
    for idx in range(len(dataset)):
        for n in n_values:

            run_dir = Path(f"results-aime/batch_{n}/problem_{idx}")
            run_dir.mkdir(parents=True, exist_ok=True)
