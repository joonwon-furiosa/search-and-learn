...
config = {
    ...
    "dataset_name": "nguyen-brat/asdiv",
    "dataset_split": "train",
...
import re
def parse_asdiv_numeric_answer(ans_field) -> str:
    """
    ASDiv 'answer' looks like ['9 (apples)'] or a similar string.
    Extract the leading numeric (int/float, allow negative), defaulting to stripped text if not found.
    """
    if isinstance(ans_field, list) and len(ans_field) > 0:
        s = ans_field[0]
    else:
        s = ans_field
    if not isinstance(s, str):
        return ""
    # remove thousands separators, then find first number
    s_clean = s.replace(",", "")
...
def main():
    ...
    for idx in range(len(dataset)):
        for n in n_values:
            ...
            run_dir = Path(f"results-asdiv/batch_{n}/problem_{idx}")
            run_dir.mkdir(parents=True, exist_ok=True)
            ...
            try:
                ...
                gold = parse_asdiv_numeric_answer(batch['answer'])
                is_correct = is_equiv(result["pred"], gold, verbose=False)
